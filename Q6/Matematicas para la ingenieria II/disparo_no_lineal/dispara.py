    ...
    for tiro in tiros:
        fig.add_trace(
            go.Scatter(
                x=tiro.data["time"],
                y=tiro.data["y"],
                name=f"{'Mejor tiro:'+str(tiro.tiro) if tiros[-1] is tiro else 'tk='+str(tiro.tiro)}",
                mode=f"{'lines+' if tiros[-1] is tiro else ''}markers",
                line_color=f'rgba({255-(255/nth)*it},{(255/nth)*it},0,{(155/nth*it)+100})'
            )
        )
        it+=1
    if len(funcion_e) != 0:
...
def main():
    ...
    # funcion_e = input("Funcion exacta(para comparar): \n> ")
